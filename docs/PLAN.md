...
Claude Code 턴 종료 시(Stop hook) "방금 사용자 요청 + 요청 시간"을 터미널에 1줄 재표시하고, `/wdis N`으로 최근 N건을 조회하는 플러그인. 무상태(현재 세션 jsonl 역방향 스캔), 외부 의존성 제로 Node(`.mjs`), 테스트는 `node --test`. MVP 범위는 Claude Code 전용이다.
...
- 완료 기준: `claude --plugin-dir /Users/mini/Github/ai-tools/what-did-i-say` 로 기동 시 플러그인이 인식되고 에러가 없다 (`claude plugin list`에 노출).
...
- 엣지 케이스: 파일 없음, 빈 파일, 손상된 JSON 라인, 사용자 프롬프트가 0건, 요청 N이 실제 건수보다 큰 경우.
- 완료 기준: `node --test scripts/` **전체 통과** (통과 수를 "N/N 통과"로 보고). 이 커밋 시점에는 아직 hook에 연결하지 않으므로 실 세션 동작 변화가 없다.

### 커밋 3 — `feat: Stop hook 출력`

- 산출: `scripts/wdis.mjs`(hook 모드 — stdin JSON에서 `transcript_path` 수신 → parser 호출 → 1줄 출력), `hooks/hooks.json`에 Stop 엔트리 추가
- 완료 기준:
  1. `node --test scripts/` 통과 유지
  2. 수동 E2E: `claude --plugin-dir <repo>` 로 새 세션 기동 → 아무 프롬프트나 1회 입력 → 턴 종료 시 요청문+시간 1줄이 터미널에 표시됨
  3. 실패 안전: 손상된 transcript_path를 넘겨도 hook이 exit 0으로 종료하고 세션을 방해하지 않음 (`echo '{"transcript_path":"/nonexistent"}' | node scripts/wdis.mjs` → 종료 코드 0)
- 주의: 현재 하네스 `Stop` 슬롯에 orca `claude-hook.sh`가 이미 등록돼 있다. 병합 동작이므로 공존은 되지만 **출력 섞임을 이 커밋에서 실측**한다 (§3 체크리스트).

### 커밋 4 — `feat: /wdis 커맨드`

- 산출: `commands/wdis.md`, `scripts/wdis.mjs`에 `--list N` 모드 추가
- 인자 규칙: `/wdis` = 기본 건수, `/wdis N` = 최근 N건. N이 숫자가 아니거나 0 이하면 기본값으로 처리하고 사유를 한 줄 안내한다.
- 완료 기준:
  1. `node --test scripts/` 통과 유지
...
`claude --plugin-dir <절대경로>` — CLI에 실존하는 옵션이며(`--plugin-dir <path>`: 디렉터리 또는 .zip을 해당 세션에만 로드, 반복 지정 가능) 세션 한정이라 전역 설정을 오염시키지 않는다. 이것을 기본 검증 경로로 삼는다.
...
| # | 항목 | 판정 |
|---|---|---|
| 1 | 파서 단위 테스트 | `node --test scripts/` N/N 통과 |
| 2 | Stop hook 1줄 출력 | 새 세션에서 턴 종료 시 요청문+시간 표시 |
| 3 | **orca `claude-hook.sh`와 Stop 슬롯 공존** | 두 훅이 함께 등록된 상태로 1회 턴 종료 → 출력이 섞이거나 잘리지 않는지 육안 확인. 섞이면 출력 형식(개행·프리픽스)을 조정하고 결과를 커밋 3 메시지에 기록 |
| 4 | 실패 안전 | 존재하지 않는 transcript 경로 → exit 0 |
| 5 | `/wdis N` 조회 | 3건 요청 → 오름차순 3건, 건수 부족 시 있는 만큼 |
| 6 | README 재현 | 문서만 보고 설치·실행 성공 |

체크리스트 3번은 이 플러그인 고유의 위험 요소이므로 **최소 1회는 반드시 실세션에서 수행**한다.
...
| **M2 — E2E 동작** | 커밋 3~4 | Stop hook 1줄 출력 + `/wdis N` 조회가 실 세션에서 동작. 체크리스트 1~5 전부 통과 |
...
| jsonl 대용량(수백 MB) | 역방향 스캔으로 필요한 만큼만 읽는다. 전체 로드 금지 — TECH_SPEC의 청크 읽기 방식을 따른다 |
